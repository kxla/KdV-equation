function EX3
clc;tic;
can1=0.9;can2=0.001;can3=0.00001;
XL=0;XR=1;h=0.01;tao=h^2;tfinal=1.0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for n=1:T1
    t(n)=(n-1)*tao;
    pusai(n)=sin(app*pi*t(n));
    dfpusai(n)=app*pi*cos(app*pi*t(n));
end
% Exact Solution
for j=1:M1
    for n=1:T1
        u(j,n)=0;
        G(j,n)=-(XR-x(j))^2*dfpusai(n)/(L^2)+2*can1*pusai(n)*(XR-x(j))/(L^2)+2*can2*(pusai(n))^2*(XR-x(j))^3/(L^4);
    end
    u(j,1)=-(XR-x(j))^2*pusai(1)/(L^2);
    u(j,2)=-(XR-x(j))^2*pusai(2)/(L^2);
end
...
D0=1/tao;
for j=1:M5
    B0(j)=can1/(24*h)-can3/(2*h^3)+can2/(24*h*L^2)*pusai(1)*(XR-x(j+3-2))^2;
    C0(j)=-can1/(3*h)+13*can3/(16*h^3)-can2/(3*h*L^2)*pusai(1)*(XR-x(j+3-1))^2;
    E0(j)=can1/(3*h)-13*can3/(16*h^3)+can2/(3*h*L^2)*pusai(1)*(XR-x(j+3+1))^2;
    F0(j)=-can1/(24*h)+can3/(2*h^3)-can2/(24*h*L^2)*pusai(1)*(XR-x(j+3+2))^2;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j=4:M5
    A(j,j-3)=A0;
...
for n=3:T1
    for j=1:M5
        B1(j)=can1/(24*h)+can2/(72*h)*(u(j+3,n-1)+u(j+3-2,n-1))-can3/(2*h^3)+can2/(24*h*L^2)*pusai(n-1)*(XR-x(j+3-2))^2;
        C1(j)=-can1/(3*h)-can2/(9*h)*(u(j+3,n-1)+u(j+3-1,n-1))+13*can3/(16*h^3)-can2/(3*h*L^2)*pusai(n-1)*(XR-x(j+3-1))^2;
        E1(j)=can1/(3*h)+can2/(9*h)*(u(j+3,n-1)+u(j+3+1,n-1))-13*can3/(16*h^3)+can2/(3*h*L^2)*pusai(n-1)*(XR-x(j+3+1))^2;
        F1(j)=-can1/(24*h)-can2/(72*h)*(u(j+3,n-1)+u(j+3+2,n-1))+can3/(2*h^3)-can2/(24*h*L^2)*pusai(n-1)*(XR-x(j+3+2))^2;;
    end
    for j=4:M5
        Aa(j,j-3)=A1;
    end
...
for j=1:M1
    for n=1:T1
        u(j,n)=u(j,n)+(XR-x(j))^2*pusai(n)/(L^2);
    end
end
save u u
toc;
