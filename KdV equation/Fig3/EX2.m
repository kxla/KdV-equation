...
XL=0;XR=1;h=0.001;tao=h/2;tfinal=1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for n=1:T1
    t(n)=(n-1)*tao;
    pusai(n)=cos(pap*pi*t(n));
    dfpusai(n)=-pap*pi*sin(pap*pi*t(n));
    dfdfpusai(n)=-pap*pi*pap*pi*cos(pap*pi*t(n));
end
% Exact Solution
for j=1:M1
    for n=1:T1
        ...
        uexat1(j,n)=uexa00+uexa11+uexa12+uexa21+uexa22+uexa23+uexa24+uexa25+uexa26;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %uexa0=pusai(n)*(1-x(j))^2;
        %uexa1=-((x(j)-1)^6)/60*((pusai(n))^2)-((x(j)-1)^5)/60*dfpusai(n)-((x(j)-1)^4)/12*pusai(n)+(x(j)^2)*(((pusai(n))^2)/60-pusai(n)/12-dfpusai(n)/60)+x(j)*(dfpusai(n)/30-pusai(n)/6-((pusai(n))^2)/30)+pusai(n)/60+pusai(n)/12-dfpusai(n)/60;
        %uexa2=1/3326400*((-1+x(j))^2*x(j)*(-66*(250-50*x(j)^6-150*x(j)+1150*x(j)^3+350*x(j)^5-930*x(j)^4-550*x(j)^2)*pusai(n)^3+48*(-102+78*x(j)+258*x(j)^2-717*x(j)^3+849*x(j)^4-588*x(j)^5+252*x(j)^6-63*x(j)^7+7*x(j)^8)*pusai(n)^4+11*pusai(n)*(15*x(j)^3*56-12*x(j)^2*280+280*3+x(j)*980*3)...
              %+4*(-91+21*x(j)+133*x(j)^2-175*x(j)^3+105*x(j)^4-35*x(j)^5+5*x(j)^6)*dfpusai(n)...
              %+22*pusai(n)^2*(150+465*x(j)+45*x(j)^5-15*x(j)^4*18+25*x(j)^3*27-900*x(j)^2+3*(64-...
              %44*x(j)-152*x(j)^2+370*x(j)^3-368*x(j)^4+196*x(j)^5-56*x(j)^6+7*x(j)^7)*dfpusai(n))...
              %-33*(10*(30-8*x(j)^4+24*x(j)-66*x(j)^2+40*x(j)^3)*dfpusai(n)...
              %+(-54+9*x(j)+72*x(j)^2-75*x(j)^3+30*x(j)^4-5*x(j)^5)*dfdfpusai(n))));
        %uexat(j,n)=uexa0+uexa1+uexa2;
        u(j,n)=0;
        uexact(j,n)=uexat1(j,n);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end
    %u(j,1)=uexat1(j,1)-(1-x(j)/L)^2*pusai(1);
    %u(j,2)=uexat1(j,2)-(1-x(j)/L)^2*pusai(2);
end
